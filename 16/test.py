...
)  # state is mutable, but we hash only on other fields
class Valve:
...
def main():
    ...
    def calc_max_relief(opened, mins_remaining, curr_valve_id, elephant=False):
        ...
        for neighbour in current_valve.leads_to:
            ...
            most_relief = max(
                most_relief,
                calc_max_relief(opened, mins_remaining - 1, neighbour, elephant),
            )

        # We only want to open valves that are closed, and where flow rate is > 0
        if (
            curr_valve_id not in opened
            and current_valve.rate > 0
            ...
            for neighbour in current_valve.leads_to:
                ...
                most_relief = max(
                    ...
                    total_released
                    + calc_max_relief(
                        frozenset(opened), mins_remaining - 1, neighbour, elephant
                    ),
                )
...
def parse_input(data) -> dict[str, Valve]:
    ...
    for line in data:
        ...
        valves[valve] = Valve(
            valve, int(rate), {x.strip() for x in leads_to.split(",")}
        )
