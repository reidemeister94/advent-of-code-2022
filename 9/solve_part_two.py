...
def is_touching(i):
    ...
    return (
        abs(ROPE[i][0] - ROPE[i + 1][0]) <= 1 and abs(ROPE[i][1] - ROPE[i + 1][1]) <= 1
    )


def move_tail(i):
    # move tail, ROPE[i] is the tail while ROPE[i+1] is the head
    global ROPE
...
def move_rope(direction):
    ...
    # move head
    ROPE[-1] = (
        ROPE[-1][0] + direction_map[direction][0],
        ROPE[-1][1] + direction_map[direction][1],
    )
